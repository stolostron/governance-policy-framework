...
# Store remaining arguments for the sync utility
sync_util_args=("$@")
...
upstream_script="${SCRIPT_PATH}/fetch-repo-list.sh"
...
read -r -p "Continue with updates? (n - exit, y - continue) " response
case "${response}" in
n)
  exit
  ;;
...
for repo in ${repos}; do
  ...
  if (${git} diff --staged --exit-code >/dev/null); then
    ...
    [DRY RUN] https://github.com/${repo}/compare/${target_branch}...${head_branch}"
      ...
      ${git} --no-pager diff --staged
      if ! ${silent}; then
        read -r -p "Continue with commit? (n - skip, y - proceed) " response
        case "${response}" in
        n)
          continue
          ;;
