...
docker run --volume $(pwd)/results:/opt/app-root/src/grc-ui/test-output/e2e --env OC_CLUSTER_URL=$OC_HUB_CLUSTER_URL --env OC_CLUSTER_PASS=$OC_HUB_CLUSTER_PASS --env OC_CLUSTER_USER=$OC_CLUSTER_USER --env RBAC_PASS=$RBAC_PASS $DOCKER_URI
