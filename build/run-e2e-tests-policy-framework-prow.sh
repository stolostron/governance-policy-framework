...
for TEST_SUITE in integration policy-collection; do
  ...
  CGO_ENABLED=0 ginkgo -v --fail-fast --slow-spec-threshold=10s --junit-report=${TEST_SUITE}.xml --output-dir=test-output test/${TEST_SUITE} -- -cluster_namespace=$MANAGED_CLUSTER_NAME || EXIT_CODE=$?
