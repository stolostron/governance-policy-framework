...
var _ = Describe("Test cert policy", func() {
	Describe("Test cert policy inform", func() {
		...
		It("should be created on managed cluster", func() {
			...
			_, err := clientHubDynamic.Resource(
				common.GvrPlacementRule).Namespace(userNamespace).UpdateStatus(context.TODO(),
				plr,
		...
		It("the policy should be noncompliant after creating a certficate that expires", func() {
			...
			utils.Kubectl(
				"apply",
				"-f",
				"../resources/cert_policy/issuer.yaml",
				"-n",
				"default",
			...
			utils.Kubectl(
				"apply",
				"-f",
				"../resources/cert_policy/certificate.yaml",
				"-n",
				"default",
				"--kubeconfig=../../kubeconfig_managed",
			)
		...
		It("the policy should be noncompliant after creating a "+
			"certficate that expires and then is compliant after a fix", func() {
			By("Creating ../resources/cert_policy/issuer.yaml in ns default")
			utils.Kubectl(
				"apply",
				"-f",
		...
		It("the policy should be noncompliant after creating a certficate that has too long duration", func() {
			...
			utils.Kubectl(
				"apply",
				"-f",
				"../resources/cert_policy/certificate_long.yaml",
				"-n",
				"default",
				"--kubeconfig=../../kubeconfig_managed",
			)
