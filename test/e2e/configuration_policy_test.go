...
var _ = Describe("Test configuration policy", func() {
	Describe("Create a policy to configure a pod", func() {
		...
		It("should be created on managed cluster", func() {
			...
			Expect(err).To(BeNil())
			By("Checking " + podPolicyName + " on managed cluster in ns " + clusterNamespace)
			managedplc := utils.GetWithTimeout(clientManagedDynamic, gvrPolicy, userNamespace+"."+podPolicyName, clusterNamespace, true, defaultTimeoutSeconds)
