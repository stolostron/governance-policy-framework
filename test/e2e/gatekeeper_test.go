...
var _ = Describe("Test gatekeeper", Ordered, func() {
	...
	Describe("Test gatekeeper policy creation", Ordered, func() {
		...
		It("should generate statuses properly on hub, no violation expected", func() {
			...
			}, defaultTimeoutSeconds*10, 1).Should(Equal("Compliant"))
			...
			Eventually(func(g Gomega) interface{} {
				...
				Equal("Compliant; notification - k8srequiredlabels [ns-must-have-gk] found as specified, " +
		...
		AfterAll(func() {
			...
			_, err := common.OcManaged(
				"delete", "K8sRequiredLabels", "--all",
				"--ignore-not-found",
			)
			Expect(err).To(BeNil())
			_, err = common.OcManaged(
				"delete", "crd", "k8srequiredlabels.constraints.gatekeeper.sh",
				"--ignore-not-found",
			)
			Expect(err).To(BeNil())
			...
			_, err = common.OcManaged(
				"delete", "events", "--all", "-n", gatekeeperNS,
				"--ignore-not-found",
			)
			Expect(err).To(BeNil())

			By("Deleting ns e2etestsuccess")
			_, err = common.OcManaged("delete", "ns", "e2etestsuccess", "--ignore-not-found")
			Expect(err).To(BeNil())
		})
	})
})
