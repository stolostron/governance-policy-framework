...
var _ = Describe("Test gatekeeper", func() {
	...
	Describe("Test gatekeeper policy creation", func() {
		...
		It("should deploy gatekeeper release on managed cluster", func() {
			configCRD := GetClusterLevelWithTimeout(
				clientManagedDynamic,
				common.GvrCRD,
			...
			cpsCRD := GetClusterLevelWithTimeout(
				clientManagedDynamic,
				common.GvrCRD,
			...
			ctpsCRD := GetClusterLevelWithTimeout(
				clientManagedDynamic,
				common.GvrCRD,
			...
			ctCRD := GetClusterLevelWithTimeout(
				clientManagedDynamic,
				common.GvrCRD,
		...
		It("configurationPolicies should be created on managed", func() {
			...
			plr.Object["status"] = utils.GeneratePlrStatus("managed")
			_, err = clientHubDynamic.Resource(common.GvrPlacementRule).Namespace("default").UpdateStatus(
				context.TODO(),
				plr,
				metav1.UpdateOptions{},
			)
			...
			admission := utils.GetWithTimeout(
				clientManagedDynamic,
				common.GvrConfigurationPolicy,
				cfgpoladmissionName,
				clusterNamespace,
		...
		It("K8sRequiredLabels ns-must-have-gk should be created on managed", func() {
			...
			k8srequiredlabelsCRD := GetClusterLevelWithTimeout(
				clientManagedDynamic,
				common.GvrCRD,
			...
			nsMustHaveGkCR := GetClusterLevelWithTimeout(
				clientManagedDynamic,
				common.GvrK8sRequiredLabels,
				"ns-must-have-gk",
				true,
				defaultTimeoutSeconds*2,
			)
		...
		It("should generate statuses properly on hub, no violation expected", func() {
			...
			Eventually(func() interface{} {
				...
				details, _ := plc.Object["status"].(map[string]interface{})["details"].([]interface{})
				history := details[1].(map[string]interface{})["history"]
				Expect(history).NotTo(BeNil())

				return history.([]interface{})[0].(map[string]interface{})["message"]
			}, defaultTimeoutSeconds, 1).Should(Equal(
				"Compliant; notification - k8srequiredlabels [ns-must-have-gk] found as specified, " +
			...
			Eventually(func() interface{} {
				...
				return history.([]interface{})[0].(map[string]interface{})["message"]
			}, defaultTimeoutSeconds, 1).Should(Equal("Compliant; notification - events in namespace gatekeeper-" +
				"system missing as expected, therefore this Object template is compliant"))
		})
		...
		It("should create relatedObjects properly on managed", func() {
			...
			Eventually(func() interface{} {
				...
				ro, _ := plc.Object["status"].(map[string]interface{})["relatedObjects"].([]interface{})
				obj := ro[0].(map[string]interface{})["object"]

				return obj.(map[string]interface{})["metadata"].(map[string]interface{})["name"]
