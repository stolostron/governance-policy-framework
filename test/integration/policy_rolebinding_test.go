...
var _ = Describe(
	"GRC: [P1][Sev1][policy-grc] Test the policy-rolebinding policy",
	...
	func() {
		...
		It("stable/"+policyRoleBindingName+" should be created on the Hub", func() {
			...
			path := "/spec/policy-templates/0/objectDefinition/spec/namespaceSelector/include"
			_, err = clientHubDynamic.Resource(common.GvrPolicy).Namespace(userNamespace).Patch(
				context.TODO(),
				policyRoleBindingName,
				k8stypes.JSONPatchType,
				[]byte(
					`[{"op": "replace", "path": "`+path+`", "value": ["`+policyRoleBindingNSName+`"]}]`,
				),
				metav1.PatchOptions{},
			)
			Expect(err).To(BeNil())
			...
			err = common.PatchPlacementRule(
				userNamespace,
				"placement-"+policyRoleBindingName,
				clusterNamespace,
				kubeconfigHub,
			)
			Expect(err).To(BeNil())
