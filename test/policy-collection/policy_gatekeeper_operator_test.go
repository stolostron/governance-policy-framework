...
var log = logf.Log.WithName("policy-collection")
...
func isOCP44() bool {
	...
	log.Info(fmt.Sprintf("OCP Version " + version))
...
var _ = Describe("", func() {
	...
	BeforeEach(func() {
		...
		if !canCreateOpenshiftNamespaces() {
			Skip("Skipping as upstream gatekeeper operator requires the ability " +
				"to create the openshift-gatekeeper-system namespace")
		}
	...
	const gatekeeperPolicyURL = "https://raw.githubusercontent.com/stolostron/policy-collection/" +
		"main/community/CM-Configuration-Management/policy-gatekeeper-operator.yaml"
	...
	Describe("RHACM4K-1692 GRC: [P1][Sev1][policy-grc] Test installing gatekeeper operator", func() {
		It("community/policy-gatekeeper-operator should be created on hub", func() {
			...
			By("Patching Policy Gatekeeper CR template with namespaceSelector to kubernetes.io/metadata.name=" +
				userNamespace)
			_, err = utils.KubectlWithOutput(
				"patch",
				"-n",
				...
				"--type=json",
				//nolint:lll
				"-p=[{\"op\": \"add\", \"path\": \"/spec/policy-templates/4/objectDefinition/spec/object-templates/0/objectDefinition/spec/webhook/namespaceSelector\","+
		...
		It("Informing community/policy-gatekeeper-operator", func() {
			Eventually(func() interface{} {
				...
				if err != nil {
					return fmt.Errorf("failed to inform the gatekeeper policy %s on name space %s: %w",
						gatekeeperPolicyName, userNamespace, err)
				}
	...
	Describe("RHACM4K-1274/RHACM4K-1282 GRC: [P1][Sev1][policy-grc] Test community/policy-gatekeeper-sample", func() {
		...
		It("Creating an invalid ns should generate a violation message", func() {
			...
			Eventually(func() interface{} {
				...
				message := history.([]interface{})[0].(map[string]interface{})["message"]
				fmt.Printf("%v\n", message)
