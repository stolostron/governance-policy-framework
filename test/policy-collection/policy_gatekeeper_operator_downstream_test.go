...
var _ = Describe("RHACM4K-3055", func() {
	var (
		...
		log                = logf.Log.WithName("policy_gatekeeper_operator_downstream_test")
	)
	...
	const gatekeeperPolicyURL = "https://raw.githubusercontent.com/stolostron/policy-collection/" +
		"main/stable/CM-Configuration-Management/policy-gatekeeper-operator-downstream.yaml"
	const gatekeeperPolicyName = "policy-gatekeeper-operator"
	const GKPolicyYaml = "https://raw.githubusercontent.com/stolostron/policy-collection/" +
		"main/community/CM-Configuration-Management/policy-gatekeeper-sample.yaml"
	const GKPolicyName = "policy-gatekeeper"
	const GKAssignPolicyYaml = "https://raw.githubusercontent.com/stolostron/policy-collection/" +
		"main/community/CM-Configuration-Management/policy-gatekeeper-image-pull-policy.yaml"
	const GKAssignPolicyName = "policy-gatekeeper-image-pull-policy"
	const GKAssignMetadataPolicyYaml = "https://raw.githubusercontent.com/stolostron/policy-collection/" +
		"main/community/CM-Configuration-Management/policy-gatekeeper-annotation-owner.yaml"
	...
	Describe("GRC: [P1][Sev1][policy-grc] Test installing gatekeeper operator", func() {
		It("stable/policy-gatekeeper-operator should be created on hub", func() {
			...
			_, err = utils.KubectlWithOutput(
				...
				"--type=json",
				//nolint:lll
				"-p=[{\"op\": \"add\", \"path\": \"/spec/policy-templates/2/objectDefinition/spec/object-templates/0/objectDefinition/spec/webhook/namespaceSelector\","+
			...
			_, err = utils.KubectlWithOutput(
				...
				"-n",
				userNamespace,
				"placementrule.apps.open-cluster-management.io/placement-"+gatekeeperPolicyName,
				"--type=json",
				//nolint:lll
		...
		It("Gatekeeper operator pod should be running", func() {
			...
			Eventually(func() interface{} {
				if i == 60*2 || i == 60*4 {
					msg := "gatekeeper operator pod still not created, deleting subscription and let it recreate"
		...
		It("Checking if validating webhook gatekeeper-validating-webhook-configuration "+
			...
			Eventually(func() interface{} {
				webhook, _ := clientManaged.AdmissionregistrationV1().ValidatingWebhookConfigurations().Get(
					context.TODO(),
					"gatekeeper-validating-webhook-configuration", metav1.GetOptions{},
				)
		...
		It("Gatekeeper audit pod should be running", func() {
			...
			Eventually(func() interface{} {
				podList, err := clientManaged.CoreV1().Pods("openshift-gatekeeper-system").List(
					context.TODO(),
					metav1.ListOptions{LabelSelector: "control-plane=audit-controller"},
				)
				Expect(err).To(BeNil())
	...
	Describe("GRC: [P1][Sev1][policy-grc] Test stable/policy-gatekeeper-sample", func() {
		It("stable/policy-gatekeeper-sample should be created on hub", func() {
			...
			_, err = utils.KubectlWithOutput(
				...
				"-n",
				userNamespace,
				"placementrule.apps.open-cluster-management.io/placement-"+GKPolicyName,
				"--type=json",
				//nolint:lll
				"-p=[{\"op\": \"replace\", \"path\": \"/spec/clusterSelector/matchExpressions\", \"value\":[{\"key\": \"name\", \"operator\": \"In\", \"values\": ["+clusterNamespace+"]}]}]",
				"--kubeconfig="+kubeconfigHub,
		...
		It("stable/policy-gatekeeper-sample should be compliant", func() {
			...
			Eventually(func() interface{} {
				...
				details, _ := plc.Object["status"].(map[string]interface{})["details"].([]interface{})
	...
	Describe("GRC: [P1][Sev1][policy-grc] Test enabling gatekeeper mutation feature", func() {
		It("Enabling mutation feature through policy", func() {
			Eventually(func() interface{} {
				...
				objectDefinition := policyTemplates.([]interface{})[2].(map[string]interface{})["objectDefinition"]
		...
		It("Enforcing policy-gatekeeper-operator to enable mutation feature", func() {
			Eventually(func() interface{} {
				...
				if err != nil {
					return fmt.Errorf("failed to enforce the gatekeeper policy %s on name space %s: %w",
						gatekeeperPolicyName, userNamespace, err)
				}
		...
		It("Checking if Assign/AssingnMetadata CRDs have been created", func() {
			...
			Eventually(func() interface{} {
				...
				defaultTimeoutSeconds*2,
				1,
			).Should(ContainSubstring("CREATED AT\nassignmetadata.mutations.gatekeeper.sh"))
		})
	...
	Describe("GRC: [P1][Sev1][policy-grc] Install mutation policy", func() {
		It("Creating mutation policy on hub", func() {
			...
			_, err = utils.KubectlWithOutput(
				...
				"-n",
				userNamespace,
				"placementrule.apps.open-cluster-management.io/placement-"+GKAssignPolicyName,
				"--type=json",
				//nolint:lll
				"-p=[{\"op\": \"replace\", \"path\": \"/spec/clusterSelector/matchExpressions\", \"value\":[{\"key\": \"name\", \"operator\": \"In\", \"values\": ["+clusterNamespace+"]}]}]",
				"--kubeconfig="+kubeconfigHub,
			...
			_, err = utils.KubectlWithOutput(
				...
				"-n",
				userNamespace,
				"placementrule.apps.open-cluster-management.io/placement-"+GKAssignMetadataPolicyName,
				"--type=json",
				//nolint:lll
				"-p=[{\"op\": \"replace\", \"path\": \"/spec/clusterSelector/matchExpressions\", \"value\":[{\"key\": \"name\", \"operator\": \"In\", \"values\": ["+clusterNamespace+"]}]}]",
				"--kubeconfig="+kubeconfigHub,
	...
	Describe("GRC: [P1][Sev1][policy-grc] Test disabling gatekeeper mutation feature", func() {
		It("Disabling mutation feature through policy", func() {
			Eventually(func() interface{} {
				...
				if err != nil {
					return fmt.Errorf("failed to disable the gatekeeper policy %s on name space %s: %w",
						gatekeeperPolicyName, userNamespace, err)
				}
	...
	Describe("GRC: [P1][Sev1][policy-grc] Clean up after all", func() {
		...
		It("Clean up stable/policy-gatekeeper-sample", func() {
			...
			_, err = utils.KubectlWithOutput("delete", "ns", "e2etestsuccess", "--kubeconfig="+kubeconfigManaged)
			Expect(err).To(BeNil())

			_, err = utils.KubectlWithOutput("delete", "ns", "e2etestfail", "--kubeconfig="+kubeconfigManaged)
			Expect(err).To(BeNil())
		})
		It("Clean up stable/policy-gatekeeper-operator", func() {
			...
			Eventually(func() interface{} {
				...
				defaultTimeoutSeconds*4,
				1,
			).Should(ContainSubstring("No resources found")) // k8s will respond with this even if the ns was deleted.
