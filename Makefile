...
e2e-test:
	@if [ -z "$(TEST_FILE)" ]; then\
		$(GOPATH)/bin/ginkgo -v $(TEST_ARGS) --slow-spec-threshold=10s --fail-fast test/e2e;\
	else\
		$(GOPATH)/bin/ginkgo -v $(TEST_ARGS) --slow-spec-threshold=10s --fail-fast --focus-file=$(TEST_FILE) test/e2e;\
	fi
...
integration-test:
	@if [ -z "$(TEST_FILE)" ]; then\
		$(GOPATH)/bin/ginkgo -v $(TEST_ARGS) --slow-spec-threshold=10s --fail-fast test/integration;\
	else\
		$(GOPATH)/bin/ginkgo -v $(TEST_ARGS) --slow-spec-threshold=10s --fail-fast --focus-file=$(TEST_FILE) test/integration;\
	fi

policy-collection-test:
	@if [ -z "$(TEST_FILE)" ]; then\
		$(GOPATH)/bin/ginkgo -v $(TEST_ARGS) --slow-spec-threshold=10s --fail-fast test/policy-collection;\
	else\
		$(GOPATH)/bin/ginkgo -v $(TEST_ARGS) --slow-spec-threshold=10s --fail-fast --focus-file=$(TEST_FILE) test/policy-collection;\
	fi
