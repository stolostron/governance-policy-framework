...
deploy-policy-framework-hub-crd-operator:
	...
	kubectl apply -f https://raw.githubusercontent.com/stolostron/governance-policy-propagator/release-2.4/deploy/crds/policy.open-cluster-management.io_policies.yaml
	kubectl apply -f https://raw.githubusercontent.com/stolostron/governance-policy-propagator/release-2.4/deploy/crds/policy.open-cluster-management.io_placementbindings.yaml
	kubectl apply -f https://raw.githubusercontent.com/stolostron/governance-policy-propagator/release-2.4/deploy/crds/policy.open-cluster-management.io_policyautomations.yaml
	@echo installing policy-propagator on hub
	kubectl apply -f https://raw.githubusercontent.com/stolostron/governance-policy-propagator/release-2.4/deploy/operator.yaml -n $(KIND_HUB_NAMESPACE)
...
deploy-policy-framework-managed-crd-operator:
	...
	kubectl apply -f https://raw.githubusercontent.com/stolostron/governance-policy-spec-sync/release-2.4/deploy/operator.yaml -n $(KIND_MANAGED_NAMESPACE)
	...
	kubectl apply -f https://raw.githubusercontent.com/stolostron/governance-policy-status-sync/release-2.4/deploy/operator.yaml -n $(KIND_MANAGED_NAMESPACE)
	...
	kubectl apply -f https://raw.githubusercontent.com/stolostron/governance-policy-template-sync/release-2.4/deploy/operator.yaml -n $(KIND_MANAGED_NAMESPACE)
...
kind-deploy-policy-framework:
	...
	kubectl apply -f https://raw.githubusercontent.com/stolostron/governance-policy-propagator/release-2.4/deploy/operator.yaml -n $(KIND_HUB_NAMESPACE) --kubeconfig=$(PWD)/kubeconfig_$(HUB_CLUSTER_NAME)
...
install-crds:
	...
	kubectl apply -f https://raw.githubusercontent.com/stolostron/multicloud-operators-placementrule/release-2.4/deploy/crds/apps.open-cluster-management.io_placementrules_crd.yaml --kubeconfig=$(PWD)/kubeconfig_$(HUB_CLUSTER_NAME)
	...
	kubectl apply -f https://raw.githubusercontent.com/stolostron/api/main/cluster/v1alpha1/0000_04_clusters.open-cluster-management.io_placementdecisions.crd.yaml --kubeconfig=$(PWD)/kubeconfig_$(HUB_CLUSTER_NAME)
	kubectl apply -f https://raw.githubusercontent.com/stolostron/api/main/cluster/v1alpha1/0000_03_clusters.open-cluster-management.io_placements.crd.yaml --kubeconfig=$(PWD)/kubeconfig_$(HUB_CLUSTER_NAME)
	kubectl apply -f https://raw.githubusercontent.com/stolostron/governance-policy-propagator/release-2.4/deploy/crds/policy.open-cluster-management.io_placementbindings.yaml --kubeconfig=$(PWD)/kubeconfig_$(HUB_CLUSTER_NAME)
	kubectl apply -f https://raw.githubusercontent.com/stolostron/governance-policy-propagator/release-2.4/deploy/crds/policy.open-cluster-management.io_policies.yaml --kubeconfig=$(PWD)/kubeconfig_$(HUB_CLUSTER_NAME)
	kubectl apply -f https://raw.githubusercontent.com/stolostron/governance-policy-propagator/release-2.4/deploy/crds/policy.open-cluster-management.io_policyautomations.yaml --kubeconfig=$(PWD)/kubeconfig_$(HUB_CLUSTER_NAME)
	@echo installing crds on managed
	kubectl apply -f https://raw.githubusercontent.com/stolostron/governance-policy-propagator/release-2.4/deploy/crds/policy.open-cluster-management.io_policies.yaml --kubeconfig=$(PWD)/kubeconfig_$(MANAGED_CLUSTER_NAME)
...
travis-slack-reporter:
	docker run --volume $(PWD)/results:/opt/app-root/src/grc-ui/test-output/e2e \
		...
		quay.io/stolostron/grc-ui-tests:latest-2.4 node ./tests/utils/slack-reporter.js
